...
class Bones {
  ...
  // ... (serializeBone, deserializeBone, saveBones, loadBones, flattenBones, findBoneById 保持不變) ...
  serializeBone(bone) {
    if (!bone) return null;
  ...
  findBoneById(bone, id) { 
      if(arguments.length === 1) { id = bone; bone = skeletons[0]; }
      if (!bone || !id) return null;
      if (Array.isArray(bone)) { for(const b of bone) { const found = this.findBoneById(b, id); if(found) return found; } return null; }
      if (bone.rootBones) return this.findBoneById(bone.rootBones, id);
      if (bone.id === id) return bone;
      if (bone.children) { for (const child of bone.children) { const found = this.findBoneById(child, id); if (found) return found; } }
      return null;
  }
  ...
  handleMeshBoneCreateMouseDown(xNDC, yNDC, isShiftPressed) {
    if (isShiftPressed && lastSelectedBone.value) {
      ...
      mousedown_x = tail.x; mousedown_y = tail.y;
    } else {
      mousedown_x = xNDC; mousedown_y = yNDC;
    }
    mousemove_x = xNDC; mousemove_y = yNDC;
  }
  meshboneCreateMouseMove(xNDC, yNDC) { mousemove_x = xNDC; mousemove_y = yNDC; }
  MeshBoneCreate(xNDC, yNDC) {
    ...
    lastSelectedBone.value = newBone; lastSelectedBonePart.value = 'tail';
    mousedown_x = null; mousedown_y = null; mousemove_x = null; mousemove_y = null;
  }
  _moveBoneHead(bone, x, y) {
    if (bone.isConnected && bone.parent) { bone.setGlobalHead(x, y); bone.parent.setGlobalTail(x, y); } else { bone.setGlobalHead(x, y); }
  }
  _moveBoneTail(bone, x, y) {
    if (bone.isConnected && bone.children.length > 0) { bone.setGlobalTail(x, y); bone.children.forEach(child => child.setGlobalHead(x, y)); } 
    else {
      const childrenState = bone.children.filter(child => !child.isConnected).map(child => ({ bone: child, head: child.getGlobalHead(), rotation: child.globalRotation }));
      bone.setGlobalTail(x, y);
      childrenState.forEach(({ bone: child, head }) => { child.setGlobalHead(head.x, head.y); child._markDirty(); });
    }
  }
  _moveBoneWhole(bone, x, y, offsetX, offsetY) {
    const originalHead = bone.getGlobalHead();
    const originalTail = bone.getGlobalTail();
    const connectedChildren = bone.children.filter(child => child.isConnected).map(child => ({ bone: child, tail: child.getGlobalTail() }));
    ...
    if (bone.isConnected && bone.parent) { bone.parent.setGlobalTail(bone.getGlobalHead().x, bone.getGlobalHead().y); }
    connectedChildren.forEach(({ bone: child, tail }) => {
      ...
      const angle = child.globalRotation;
  ...
  meshBoneEditMouseMove(xNDC, yNDC) {
    ...
    else if (part === 'middle') { if (mousedown_x !== null && mousedown_y !== null) this._moveBoneWhole(bone, xNDC, yNDC, bone.offsetX, bone.offsetY); }
  }
  ...
  handleSelectPointsMouseUp(xNDC, yNDC, layerIndex, isShiftPressed = false, isCtrlPressed = false) {
    ...
    for (let i = 0; i < vertices.length; i += 4) {
      // 🔥 [修正] 強制 z=0, w=1
      // 避免 vertices[i+2] (u) 和 vertices[i+3] (v) 被當作 z, w 參與運算
      const localVert = [vertices[i], vertices[i + 1], 0, 1];
  ...
  updatePoseMesh(gl) {
    ...
    const boneMap = {};
    const collectBones = (bone) => { boneMap[bone.name] = bone; bone.children.forEach(collectBones); };
    ...
    const deformVertex = (vx, vy, vz, vw, bone, weight, width, height, top, left) => {
      ...
      const cosR = Math.cos(rotationDelta), sinR = Math.sin(rotationDelta);
    ...
    for (const layer of layers) {
      ...
      for (const group of vertexGroups) {
        ...
        for (const v of group.vertices) {
          if (!vertexInfluenceMap.has(v.id)) vertexInfluenceMap.set(v.id, []);
      ...
      for (let i = 0; i < vertexCount; i++) {
        ...
        if (!influences || influences.length === 0) {
          newVertices[idx] = vertices[idx]; newVertices[idx + 1] = vertices[idx + 1];
          newVertices[idx + 2] = vertices[idx + 2]; newVertices[idx + 3] = vertices[idx + 3];
          continue;
        }
        ...
        for (const inf of influences) {
          ...
          const d = deformVertex(vertices[idx], vertices[idx + 1], vertices[idx + 2], vertices[idx + 3], inf.bone, normalizedWeight, width, height, top, left);
          accX += d.x; accY += d.y; accZ += d.z; accW += d.w;
        }
        newVertices[idx] = accX; newVertices[idx + 1] = accY; newVertices[idx + 2] = accZ; newVertices[idx + 3] = accW;
      }
      layer.poseVertices.value = newVertices;
      gl.bindBuffer(gl.ARRAY_BUFFER, layer.vbo);
      gl.bufferData(gl.ARRAY_BUFFER, newVertices, gl.STATIC_DRAW);
    ...
    skeletons.forEach(skeleton => {
      skeleton.forEachBone(bone => {
        ...
        bone.slots.forEach(slot => {
          ...
          const hasWeights = layer.vertexGroup && layer.vertexGroup.value && layer.vertexGroup.value.length > 0;
          if (hasWeights) {
          ...
          if (layer && layer.transformParams) {
            ...
            layer.poseTransformParams = {
              ...
              width, height, rotation: newRotation, canvasWidth, canvasHeight,
